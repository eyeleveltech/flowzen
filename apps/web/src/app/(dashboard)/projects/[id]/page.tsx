...
export default function ProjectDetailPage() {
  ...
  useEffect(() => {
    fetchProject();
    api.get<{ clients: {id: string, name: string}[] }>('/clients?limit=100').then((res) => setClients(res.clients)).catch(() => {});
      ...
      {tab === 'milestones' && (
        <div className="space-y-3">
          {project.milestones?.map((m) => (
            <div key={m.id} className={`flex items-center gap-4 rounded-2xl border border-[#E5E7EB] bg-white p-4 ${m.completed ? 'opacity-60' : ''}`}>
              <div className={`h-3 w-3 rounded-full ${m.completed ? 'bg-emerald-500' : 'bg-[#E5E7EB]'}`} />
              <div className="flex-1">
                <p className={`text-sm font-medium ${m.completed ? 'text-[#9CA3AF] line-through' : 'text-[#111827]'}`}>{m.name}</p>
              </div>
              <span className="text-xs text-[#6B7280]">{formatDate(m.dueDate)}</span>
            </div>
          ))}
        </div>
      )}
