'use client';

import { useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { api } from '@/lib/api';
import { formatCurrency, formatDate } from '@/lib/utils';
import { FileText, Plus } from 'lucide-react';
import { ExpenseFormModal } from './components/ExpenseFormModal';

export default function ExpensesPage() {
  const [showModal, setShowModal] = useState(false);

  const { data, isLoading, refetch } = useQuery({
    queryKey: ['expenses'],
    queryFn: () => api.get<any[]>('/revenue/expenses'),
  });
  const expenses = data || [];

  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-semibold text-primary tracking-tight">Expenses</h1>
          <p className="text-sm text-secondary mt-1">Track internal and external project expenses</p>
        </div>
        <button
          onClick={() => setShowModal(true)}
          className="inline-flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-[#1F2937] transition-colors"
        >
          <Plus className="h-4 w-4" />
          Log Expense
        </button>
      </div>

      {showModal && (
        <ExpenseFormModal 
          onClose={() => setShowModal(false)} 
          onSaved={() => {
            setShowModal(false);
            refetch();
          }} 
        />
      )}

      <div className="bg-white rounded-2xl border border-border overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead>
              <tr className="border-b border-border text-left text-xs font-semibold text-secondary uppercase tracking-wider">
                <th className="px-6 py-3">Vendor / Desc</th>
                <th className="px-6 py-3">Category</th>
                <th className="px-6 py-3">Date</th>
                <th className="px-6 py-3">Project / Company</th>
                <th className="px-6 py-3 text-right">Amount</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {isLoading ? (
                <tr><td colSpan={5} className="px-6 py-10 text-center text-sm text-secondary">Loading…</td></tr>
              ) : expenses.length === 0 ? (
                <tr><td colSpan={5} className="px-6 py-12 text-center text-sm text-secondary">No expenses found.</td></tr>
              ) : expenses.map((e) => (
                <tr key={e.id} className="hover:bg-surface transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <FileText className="h-4 w-4 text-[#9CA3AF] shrink-0" />
                      <div>
                        <span className="text-sm font-medium text-primary block">{e.vendor}</span>
                        {e.description && <span className="text-[11px] text-secondary">{e.description}</span>}
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 text-sm text-secondary">{e.category}</td>
                  <td className="px-6 py-4 text-sm text-secondary">{formatDate(e.date)}</td>
                  <td className="px-6 py-4 text-sm text-secondary">
                    {e.project?.name || e.client?.name || '-'}
                  </td>
                  <td className="px-6 py-4 text-sm font-medium text-primary text-right">{formatCurrency(Number(e.amount))}</td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      </div>
    </div>
  );
}
