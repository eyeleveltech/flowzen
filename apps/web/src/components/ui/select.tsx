      ...
      >
        <span className="flex items-center gap-2 min-w-0">
          ...
          {selectedOption?.avatar && (
            <span className="h-5 w-5 shrink-0 rounded-full bg-subtle border border-border text-primary text-[9px] font-semibold flex items-center justify-center">{selectedOption.avatar}</span>
          )}
                ...
                >
                  ...
                  {filteredOptions.map((option) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary/10 focus:bg-surface ${value === option.value
                    ...
                    >
                      <span className="flex items-center gap-2.5">
                        {option.avatar && (
                          <span className="h-7 w-7 shrink-0 rounded-full bg-subtle border border-border text-primary text-[10px] font-semibold flex items-center justify-center">{option.avatar}</span>
                        )}
                        <span className="min-w-0 flex-1">
                          <span className="block truncate">{option.label}</span>
                          {option.sublabel && <span className="block text-xs text-secondary truncate">{option.sublabel}</span>}
