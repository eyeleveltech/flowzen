        ...
        {canSwitch && (
          <Link href="/modules">
            <div className={cn('group flex items-center gap-3 rounded-xl px-3 py-2.5 mb-2 text-sm font-medium border border-border bg-[#F9FAFB] text-secondary hover:text-primary hover:border-[#D1D5DB] transition-all', sidebarCollapsed && 'justify-center px-0')}>
              <ArrowLeftRight className="h-4.5 w-4.5 shrink-0 text-muted group-hover:text-primary" />
              <AnimatePresence>
                {!sidebarCollapsed && (
        ...
        {visibleNav.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              ...
              >
                <item.icon className={cn('h-4.5 w-4.5 shrink-0', isActive ? 'text-white' : 'text-muted group-hover:text-primary')} />
                <AnimatePresence>
                  {!sidebarCollapsed && (
                    <motion.span
        ...
        {BOTTOM_NAV_ITEMS.filter(item => item.href !== '/profile' && (!item.roles || item.roles.includes(user?.role || ''))).map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              ...
              >
                <item.icon className="h-4.5 w-4.5 shrink-0 text-muted group-hover:text-primary" />
                <AnimatePresence>
                  {!sidebarCollapsed && (
                    <motion.span
                      initial={{ opacity: 0 }}
