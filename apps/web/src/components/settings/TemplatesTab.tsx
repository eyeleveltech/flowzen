  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        >
          <Icon as={Plus} size="md" /> Create Template
        </button>
      </div>
      ...
      <AnimatePresence>
        {showModal && (
          <div className="fixed inset-0 bg-black/20 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-2xl shadow-modal shadow-black/10 w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
              ...
              <div className="p-4 sm:p-6 overflow-y-auto">
                <form id="template-form" onSubmit={handleSave} className="space-y-6">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea id="tpl-description" rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full bg-white border border-border rounded-xl px-4 py-2.5 text-sm font-medium text-primary focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/25 focus-visible:ring-offset-1 outline-none transition-colors duration-150 motion-reduce:transition-none resize-none" />
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    {form.structure.tasks.map((task, index) => (
                      <div key={index} className="bg-surface p-3.5 sm:p-4 rounded-xl border border-border space-y-4 relative">
                        <button type="button" onClick={() => removeTask(index)} className="absolute top-4 right-4 text-secondary hover:text-red-600"><Icon as={Trash2} size="md" /></button>
                        <div className="space-y-1.5 w-[85%] sm:w-[90%]">
                          ...
                          <input id={`task-${index}-title`} required value={task.title} onChange={(e) => updateTask(index, 'title', e.target.value)} className="w-full bg-white border border-border rounded-lg px-3 py-2 text-sm font-medium text-primary focus-visible:border-primary focus-visible:ring-2 focus-visible:ring-primary/25 focus-visible:ring-offset-1 outline-none" />
                        </div>
