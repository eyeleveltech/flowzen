...
2. **HIGH — Lead form loses everything on a misclick.** ~20 fields; backdrop click/X discards silently; no Escape, no dirty-guard (`LeadModal.tsx:220`). Also requires name+email+10-digit phone — reps often have only a handle or number. → confirm-on-dirty-close; require name + (email OR phone).
...
17. **MED — Refetch-the-world after every mutation** — dashboard binds 7 SSE events to a 6-9-call `fetchAll` + 60s poll; busy teams will feel the jank.
